...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = 'Calendar',
        ...
        template =  ''+
                        ...
                        '<span class="sel arrow" id="last"><i class="icon-arrow-left"></i></span>'+

                        
                        '<span class="year-month"></span>'+

                        '<span class="sel arrow" id="next"><i class="icon-arrow-right"></i></span>'+
                        '</div>' +
    ...
    Plugin.prototype.renderEvents = function (events, elem) {
        ...
        for(var i=1; i<=daysInMonth[live_date.getMonth()]; i++){
            $.each(events.event, function(index){
                ...
                if( event_date.getDate() == view_date.getDate()
                    ...
                    elem.parent('div:first').find('#day_' + (i+1))
                    .removeClass("day")
                    .addClass('holiday')
                    
                    
                    // .append('<span style="display:none;" class="event-title">' + this.title + '</span>')
        ...
        $('.holiday').each(function() {
                ...
                $(this).hover(function() {
                    $(this).trigger({
                        type: 'hoverOnEventDay',
                        element:this,
                        day: this.getAttribute("day"),
                        month: Number(this.getAttribute("month")),
                        year: this.getAttribute("year"),
                    });
                },
                function() {
                    $(this).trigger({
                        type: 'hoverOffEventDay',
                        element:this,
                        day: this.getAttribute("day"),
                        month: Number(this.getAttribute("month")),
                        year: this.getAttribute("year"),
    ...
    Plugin.prototype.renderCalendar = function (date) {
        ...
        this.element.parent('div:first').find('.year-month').empty();
        ...
        this.element.parent('div:first').find('.year-month').append(this.months[mon.getMonth()].toUpperCase() + ' ' + mon.getFullYear());
    ...
    Plugin.prototype.click = function (e) {
        e.stopPropagation();
            e.preventDefault();
            var target = $(e.target).closest('td, th,span');
